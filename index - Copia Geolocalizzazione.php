...
<main>
    ...
    <div class="card-info">     
        ...
        <style>
            .info-map-container { font-family: sans-serif; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; }
            #map { height: 300px; width: 100%; }
            #status-bar { padding: 10px; background: #f8f9fa; font-size: 14px; border-bottom: 1px solid #ddd; }
        </style>

        <div class="info-map-container">
            <div id="status-bar">Caricamento mappa...</div>
            <div id="map"></div>
        </div>

        <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
        <script>
            // --- COORDINATE DESTINAZIONE (Via Picentino, 23, Salerno) ---
            const destLat = 40.64379;
            const destLng = 14.86524;
            const destCoords = [destLat, destLng];

            // Inizializza la mappa sulla destinazione come impostazione predefinita
            var map = L.map('map').setView(destCoords, 15);

            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '© OpenStreetMap'
            }).addTo(map);

            // Marker sulla destinazione
            L.marker(destCoords).addTo(map)
                .bindPopup("<b>Via Picentino, 23</b><br>84131 Salerno (SA)")
                .openPopup();
            ...
            function getDistance(lat1, lon1, lat2, lon2) {
                ...
                return (R * c).toFixed(1); // Distanza con un decimale
            }
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        ...
                        // Aggiorna il testo con la distanza
                        document.getElementById('status-bar').innerHTML = 
                            "📍 La tua posizione è stata rilevata. Distanza dal luogo: <b>" + dist + " km</b>";

                        // Aggiunge marker utente e adatta la mappa per vedere entrambi i punti
                        L.circleMarker([userLat, userLng], {radius: 8, color: 'blue'}).addTo(map).bindPopup("Tu sei qui");
                        
                        var bounds = L.latLngBounds([userLat, userLng], destCoords);
                        map.fitBounds(bounds, {padding: [50, 50]});
                    },
                    function(error) {
                        // Se l'utente nega il permesso o c'è un errore
                        document.getElementById('status-bar').innerHTML = "Visualizzazione destinazione (posizione non condivisa).";
                    }
                );
            } else {
                document.getElementById('status-bar').innerHTML = "Il tuo browser non supporta la geolocalizzazione.";
            }
        </script>
